#!/usr/bin/env python3
"""
Pennsylvania Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Pennsylvania Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://crashinfo.penndot.pa.gov
Feature Server: https://gis.penndot.pa.gov/arcgis/rest/services/CrashData/FeatureServer/0

Usage:
    python download_pennsylvania_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_pennsylvania_crash_data.py --gzip
    python download_pennsylvania_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gis.penndot.pa.gov/arcgis/rest/services/CrashData/FeatureServer/0"
...
log = logging.getLogger("pennsylvania_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Pennsylvania ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Pennsylvania crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/PennsylvaniaDOT")
    ...
    log.info("=" * 60)
    log.info(f"Pennsylvania Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
