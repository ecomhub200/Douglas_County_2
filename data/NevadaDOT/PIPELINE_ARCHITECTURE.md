# Nevada Crash Data Pipeline Architecture

> **Purpose:** Reference guide for downloading, converting, and storing Nevada crash data in CRASH LENS standardized format with Cloudflare R2 storage.

---

## 1. Data Source

- **DOT:** Nevada Department of Transportation (NDOT)
...
| Query endpoint | `https://data-ndot.opendata.arcgis.com/datasets/crashes/FeatureServer/0/query` |
...
```bash
python data/NevadaDOT/download_nevada_crash_data.py \
  --jurisdiction <county> \
  --years 2023 2024 \
  --gzip \
  --data-dir data/NevadaDOT
```
...
| `--data-dir` | Output directory (default: `data/NevadaDOT`) |
...
See `config.json` for the complete mapping from Nevada raw fields to CRASH LENS standard (VDOT reference format).
...
- **R2 upload:** Gzip CSVs to `nevada/{jurisdiction}/`
