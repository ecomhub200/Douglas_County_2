#!/usr/bin/env python3
"""
Georgia Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Georgia Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://gdot.aashtowaresafety.net
Feature Server: https://gdot.aashtowaresafety.net/arcgis/rest/services/CrashData/FeatureServer/0

Usage:
    python download_georgia_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_georgia_crash_data.py --gzip
    python download_georgia_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gdot.aashtowaresafety.net/arcgis/rest/services/CrashData/FeatureServer/0"
...
log = logging.getLogger("georgia_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Georgia ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Georgia crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/GeorgiaDOT")
    ...
    log.info("=" * 60)
    log.info(f"Georgia Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
