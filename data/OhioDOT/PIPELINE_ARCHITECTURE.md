# Ohio Crash Data Pipeline Architecture

> **Purpose:** Reference guide for downloading, converting, and storing Ohio crash data in CRASH LENS standardized format with Cloudflare R2 storage.

---

## 1. Data Source

- **DOT:** Ohio Department of Transportation (ODOT)
...
| Query endpoint | `https://gis.dot.state.oh.us/arcgis/rest/services/TIMS/CrashData/FeatureServer/0/query` |
...
```bash
python data/OhioDOT/download_ohio_crash_data.py \
  --jurisdiction <county> \
  --years 2023 2024 \
  --gzip \
  --data-dir data/OhioDOT
```
...
| `--data-dir` | Output directory (default: `data/OhioDOT`) |
...
See `config.json` for the complete mapping from Ohio raw fields to CRASH LENS standard (VDOT reference format).
...
- **R2 upload:** Gzip CSVs to `ohio/{jurisdiction}/`
