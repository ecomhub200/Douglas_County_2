#!/usr/bin/env python3
"""
Ohio Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Ohio Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://gis.dot.state.oh.us
Feature Server: https://gis.dot.state.oh.us/arcgis/rest/services/TIMS/CrashData/FeatureServer/0

Usage:
    python download_ohio_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_ohio_crash_data.py --gzip
    python download_ohio_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gis.dot.state.oh.us/arcgis/rest/services/TIMS/CrashData/FeatureServer/0"
...
log = logging.getLogger("ohio_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Ohio ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Ohio crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/OhioDOT")
    ...
    log.info("=" * 60)
    log.info(f"Ohio Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
