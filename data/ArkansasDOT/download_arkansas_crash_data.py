#!/usr/bin/env python3
"""
Arkansas Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Arkansas Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://gis.arkansas.gov
Feature Server: https://gis.arkansas.gov/arcgis/rest/services/ARDOT/CrashData/FeatureServer/0

Usage:
    python download_arkansas_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_arkansas_crash_data.py --gzip
    python download_arkansas_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gis.arkansas.gov/arcgis/rest/services/ARDOT/CrashData/FeatureServer/0"
...
log = logging.getLogger("arkansas_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Arkansas ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Arkansas crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/ArkansasDOT")
    ...
    log.info("=" * 60)
    log.info(f"Arkansas Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
