# ArkansasDOT (Arkansas Department of Transportation) — Pipeline Instructions
...
| **FIPS** | 05 |
| **DOT Name** | ArDOT |
| **Full Name** | Arkansas Department of Transportation |
| **Counties** | 75 |

## Data Source

- **System:** ArcGIS GeoServices API at `gis.arkansas.gov`
- **Portal:** `gis.arkansas.gov`
- **API URL:** `https://gis.arkansas.gov/arcgis/rest/services/ARDOT/CrashData/FeatureServer/0`
...
See `config.json` for the complete column mapping from Arkansas raw fields to CRASH LENS standardized format (VDOT reference).

**Key fields to map:** ID, DATE, LAT/LON, SEVERITY, ROUTE, COLLISION, WEATHER, LIGHT, COUNTY/FIPS

## Severity Mapping

Check `config.json` → `derivedFields.SEVERITY` for the Arkansas-specific severity derivation method.

## Jurisdiction Filtering

- 75 jurisdictions defined in `source_manifest.json`
...
```
crash-lens-data/arkansas/{jurisdiction}/
  all_roads.csv.gz
  county_roads.csv.gz
...
| `PIPELINE_ARCHITECTURE.md` | Arkansas-specific pipeline reference guide |
| `download_arkansas_crash_data.py` | Production download script |
...
- **R2 upload:** Gzip-compressed CSVs to `arkansas/{jurisdiction}/`
