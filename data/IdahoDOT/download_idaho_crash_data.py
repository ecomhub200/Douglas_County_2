#!/usr/bin/env python3
"""
Idaho Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Idaho Transportation Department via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data-iplan.opendata.arcgis.com
Feature Server: https://data-iplan.opendata.arcgis.com/datasets/crashes/FeatureServer/0

Usage:
    python download_idaho_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_idaho_crash_data.py --gzip
    python download_idaho_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://data-iplan.opendata.arcgis.com/datasets/crashes/FeatureServer/0"
...
log = logging.getLogger("idaho_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Idaho ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Idaho crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/IdahoDOT")
    ...
    log.info("=" * 60)
    log.info(f"Idaho Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
