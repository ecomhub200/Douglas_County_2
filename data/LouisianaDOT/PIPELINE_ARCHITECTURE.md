# Louisiana Crash Data Pipeline Architecture

> **Purpose:** Reference guide for downloading, converting, and storing Louisiana crash data in CRASH LENS standardized format with Cloudflare R2 storage.

---

## 1. Data Source

- **DOT:** Louisiana Department of Transportation and Development (LADOTD)
...
| Query endpoint | `https://data-ladotd.opendata.arcgis.com/datasets/crash-data/FeatureServer/0/query` |
...
```bash
python data/LouisianaDOT/download_louisiana_crash_data.py \
  --jurisdiction <county> \
  --years 2023 2024 \
  --gzip \
  --data-dir data/LouisianaDOT
```
...
| `--data-dir` | Output directory (default: `data/LouisianaDOT`) |
...
See `config.json` for the complete mapping from Louisiana raw fields to CRASH LENS standard (VDOT reference format).
...
- **R2 upload:** Gzip CSVs to `louisiana/{jurisdiction}/`
