#!/usr/bin/env python3
"""
Louisiana Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Louisiana Department of Transportation and Development via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data-ladotd.opendata.arcgis.com
Feature Server: https://data-ladotd.opendata.arcgis.com/datasets/crash-data/FeatureServer/0

Usage:
    python download_louisiana_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_louisiana_crash_data.py --gzip
    python download_louisiana_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://data-ladotd.opendata.arcgis.com/datasets/crash-data/FeatureServer/0"
...
log = logging.getLogger("louisiana_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Louisiana ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Louisiana crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/LouisianaDOT")
    ...
    log.info("=" * 60)
    log.info(f"Louisiana Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
