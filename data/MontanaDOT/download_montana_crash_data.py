#!/usr/bin/env python3
"""
Montana Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Montana Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://gis-mdt.opendata.arcgis.com
Feature Server: https://gis-mdt.opendata.arcgis.com/datasets/crashes/FeatureServer/0

Usage:
    python download_montana_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_montana_crash_data.py --gzip
    python download_montana_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gis-mdt.opendata.arcgis.com/datasets/crashes/FeatureServer/0"
...
log = logging.getLogger("montana_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Montana ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Montana crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/MontanaDOT")
    ...
    log.info("=" * 60)
    log.info(f"Montana Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
