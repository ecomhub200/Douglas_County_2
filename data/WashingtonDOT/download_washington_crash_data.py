#!/usr/bin/env python3
"""
Washington Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Washington State Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://geo.wa.gov
Feature Server: https://geo.wa.gov/datasets/wsdot-crash-data/FeatureServer/0

Usage:
    python download_washington_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_washington_crash_data.py --gzip
    python download_washington_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://geo.wa.gov/datasets/wsdot-crash-data/FeatureServer/0"
...
log = logging.getLogger("washington_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Washington ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Washington crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/WashingtonDOT")
    ...
    log.info("=" * 60)
    log.info(f"Washington Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
