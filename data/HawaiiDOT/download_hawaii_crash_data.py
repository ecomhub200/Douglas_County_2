#!/usr/bin/env python3
"""
Hawaii Crash Data — Socrata SODA API Downloader

Downloads crash data from Hawaii Department of Transportation via Socrata SODA API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data.hawaii.gov
API: https://data.hawaii.gov/resource/a393-uawk.json

Usage:
    python download_hawaii_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_hawaii_crash_data.py --gzip
    python download_hawaii_crash_data.py --health-check
"""
...
SOCRATA_BASE_URL = "https://data.hawaii.gov/resource/a393-uawk.json"
SOCRATA_METADATA_URL = "https://data.hawaii.gov/api/views/a393-uawk.json"
...
log = logging.getLogger("hawaii_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Hawaii Socrata API — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Hawaii crash data from Socrata SODA API")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/HawaiiDOT")
    ...
    log.info("=" * 60)
    log.info(f"Hawaii Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
