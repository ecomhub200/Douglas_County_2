#!/usr/bin/env python3
"""
New York City Crash Data — Socrata SODA API Downloader

Downloads crash data from New York City Department of Transportation via Socrata SODA API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data.cityofnewyork.us
API: https://data.cityofnewyork.us/resource/h9gi-nx95.json

Usage:
    python download_nyc_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_nyc_crash_data.py --gzip
    python download_nyc_crash_data.py --health-check
"""
...
SOCRATA_BASE_URL = "https://data.cityofnewyork.us/resource/h9gi-nx95.json"
SOCRATA_METADATA_URL = "https://data.cityofnewyork.us/api/views/h9gi-nx95.json"
...
log = logging.getLogger("nyc_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("New York City Socrata API — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download New York City crash data from Socrata SODA API")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/NYCDOT")
    ...
    log.info("=" * 60)
    log.info(f"New York City Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
