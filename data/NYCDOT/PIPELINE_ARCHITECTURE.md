# New York City Crash Data Pipeline Architecture

> **Purpose:** Reference guide for downloading, converting, and storing New York City crash data in CRASH LENS standardized format with Cloudflare R2 storage.

---

## 1. Data Source

- **DOT:** New York City Department of Transportation (NYC DOT)
...
| Parameter | Value |
|-----------|-------|
| Base URL | `https://data.cityofnewyork.us/resource/h9gi-nx95.json` |
...
```bash
python data/NYCDOT/download_nyc_crash_data.py \
  --jurisdiction <county> \
  --years 2023 2024 \
  --gzip \
  --data-dir data/NYCDOT
```
...
| `--data-dir` | Output directory (default: `data/NYCDOT`) |
...
See `config.json` for the complete mapping from New York City raw fields to CRASH LENS standard (VDOT reference format).
...
- **R2 upload:** Gzip CSVs to `nyc/{jurisdiction}/`
