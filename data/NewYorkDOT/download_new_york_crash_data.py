#!/usr/bin/env python3
"""
New York Crash Data — Socrata SODA API Downloader

Downloads crash data from New York State Department of Transportation via Socrata SODA API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data.ny.gov
API: https://data.ny.gov/resource/e8ky-4vqe.json

Usage:
    python download_new_york_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_new_york_crash_data.py --gzip
    python download_new_york_crash_data.py --health-check
"""
...
SOCRATA_BASE_URL = "https://data.ny.gov/resource/e8ky-4vqe.json"
SOCRATA_METADATA_URL = "https://data.ny.gov/api/views/e8ky-4vqe.json"
...
log = logging.getLogger("new_york_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("New York Socrata API — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download New York crash data from Socrata SODA API")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/NewYorkDOT")
    ...
    log.info("=" * 60)
    log.info(f"New York Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
