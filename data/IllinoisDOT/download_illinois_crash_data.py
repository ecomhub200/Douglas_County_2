#!/usr/bin/env python3
"""
Illinois Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Illinois Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://gis-idot.opendata.arcgis.com
Feature Server: https://gis-idot.opendata.arcgis.com/datasets/crashes/FeatureServer/0

Usage:
    python download_illinois_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_illinois_crash_data.py --gzip
    python download_illinois_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gis-idot.opendata.arcgis.com/datasets/crashes/FeatureServer/0"
...
log = logging.getLogger("illinois_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Illinois ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Illinois crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/IllinoisDOT")
    ...
    log.info("=" * 60)
    log.info(f"Illinois Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
