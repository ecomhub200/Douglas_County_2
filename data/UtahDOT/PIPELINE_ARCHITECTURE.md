# Utah Crash Data Pipeline Architecture

> **Purpose:** Reference guide for downloading, converting, and storing Utah crash data in CRASH LENS standardized format with Cloudflare R2 storage.

---

## 1. Data Source

- **DOT:** Utah Department of Transportation (UDOT)
...
| Query endpoint | `https://data-uplan.opendata.arcgis.com/datasets/crashes/FeatureServer/0/query` |
...
```bash
python data/UtahDOT/download_utah_crash_data.py \
  --jurisdiction <county> \
  --years 2023 2024 \
  --gzip \
  --data-dir data/UtahDOT
```
...
| `--data-dir` | Output directory (default: `data/UtahDOT`) |
...
See `config.json` for the complete mapping from Utah raw fields to CRASH LENS standard (VDOT reference format).
...
- **R2 upload:** Gzip CSVs to `utah/{jurisdiction}/`
