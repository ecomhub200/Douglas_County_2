#!/usr/bin/env python3
"""
Utah Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Utah Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data-uplan.opendata.arcgis.com
Feature Server: https://data-uplan.opendata.arcgis.com/datasets/crashes/FeatureServer/0

Usage:
    python download_utah_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_utah_crash_data.py --gzip
    python download_utah_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://data-uplan.opendata.arcgis.com/datasets/crashes/FeatureServer/0"
...
log = logging.getLogger("utah_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Utah ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Utah crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/UtahDOT")
    ...
    log.info("=" * 60)
    log.info(f"Utah Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
