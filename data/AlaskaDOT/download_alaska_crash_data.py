#!/usr/bin/env python3
"""
Alaska Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Alaska Department of Transportation and Public Facilities via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://data-soa-akdot.opendata.arcgis.com
Feature Server: https://data-soa-akdot.opendata.arcgis.com/datasets/crash-data/FeatureServer/0

Usage:
    python download_alaska_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_alaska_crash_data.py --gzip
    python download_alaska_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://data-soa-akdot.opendata.arcgis.com/datasets/crash-data/FeatureServer/0"
...
log = logging.getLogger("alaska_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Alaska ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Alaska crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/AlaskaDOT")
    ...
    log.info("=" * 60)
    log.info(f"Alaska Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
