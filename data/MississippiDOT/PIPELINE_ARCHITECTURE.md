# Mississippi Crash Data Pipeline Architecture

> **Purpose:** Reference guide for downloading, converting, and storing Mississippi crash data in CRASH LENS standardized format with Cloudflare R2 storage.

---

## 1. Data Source

- **DOT:** Mississippi Department of Transportation (MDOT)
...
| Query endpoint | `https://gis-mdot.opendata.arcgis.com/datasets/crashes/FeatureServer/0/query` |
...
```bash
python data/MississippiDOT/download_mississippi_crash_data.py \
  --jurisdiction <county> \
  --years 2023 2024 \
  --gzip \
  --data-dir data/MississippiDOT
```
...
| `--data-dir` | Output directory (default: `data/MississippiDOT`) |
...
See `config.json` for the complete mapping from Mississippi raw fields to CRASH LENS standard (VDOT reference format).
...
- **R2 upload:** Gzip CSVs to `mississippi/{jurisdiction}/`
