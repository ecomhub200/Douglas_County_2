# OklahomaDOT (Oklahoma Department of Transportation) — Pipeline Instructions
...
| **FIPS** | 40 |
| **DOT Name** | OKDOT |
| **Full Name** | Oklahoma Department of Transportation |
| **Counties** | 77 |

## Data Source

- **System:** ArcGIS GeoServices API at `gis-okdot.opendata.arcgis.com`
- **Portal:** `gis-okdot.opendata.arcgis.com`
- **API URL:** `https://gis-okdot.opendata.arcgis.com/datasets/crashes/FeatureServer/0`
...
See `config.json` for the complete column mapping from Oklahoma raw fields to CRASH LENS standardized format (VDOT reference).

**Key fields to map:** ID, DATE, LAT/LON, SEVERITY, ROUTE, COLLISION, WEATHER, LIGHT, COUNTY/FIPS

## Severity Mapping

Check `config.json` → `derivedFields.SEVERITY` for the Oklahoma-specific severity derivation method.

## Jurisdiction Filtering

- 77 jurisdictions defined in `source_manifest.json`
...
```
crash-lens-data/oklahoma/{jurisdiction}/
  all_roads.csv.gz
  county_roads.csv.gz
...
| `PIPELINE_ARCHITECTURE.md` | Oklahoma-specific pipeline reference guide |
| `download_oklahoma_crash_data.py` | Production download script |
...
- **R2 upload:** Gzip-compressed CSVs to `oklahoma/{jurisdiction}/`
