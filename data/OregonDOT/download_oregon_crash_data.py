#!/usr/bin/env python3
"""
Oregon Crash Data — ArcGIS Feature Server Downloader

Downloads crash data from Oregon Department of Transportation via ArcGIS GeoServices REST API.
Supports jurisdiction filtering, date range queries, gzip compression for R2 storage.

Data Source: https://oregon.gov/odot
Feature Server: https://gis.odot.state.or.us/arcgis/rest/services/CrashData/FeatureServer/0

Usage:
    python download_oregon_crash_data.py --jurisdiction <county> --years 2023 2024
    python download_oregon_crash_data.py --gzip
    python download_oregon_crash_data.py --health-check
"""
...
FEATURE_SERVER_URL = "https://gis.odot.state.or.us/arcgis/rest/services/CrashData/FeatureServer/0"
...
log = logging.getLogger("oregon_downloader")
...
def health_check():
    ...
    log.info("=" * 60)
    log.info("Oregon ArcGIS Feature Server — Health Check")
    log.info("=" * 60)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Download Oregon crash data from ArcGIS Feature Server")
    parser.add_argument("--jurisdiction", "-j", type=str, default=None)
    parser.add_argument("--years", "-y", type=int, nargs="+", default=None)
    parser.add_argument("--data-dir", "-d", type=str, default="data/OregonDOT")
    ...
    log.info("=" * 60)
    log.info(f"Oregon Crash Data Downloader")
    log.info("=" * 60)
    start = time.time()
